...
main(int argc, char *argv[])
{
    ...
    getopt_add_int(gopt, '\0', "LED", "0", "Set the servo's led on (0-7 XL, 0,1 AX/MX)");
    getopt_add_int(gopt, '\0', "KP", "32", "Set the servo's KP (MX/XL)");
    getopt_add_int(gopt, '\0', "KD", "0", "Set the servo's KD (MX/XL)");
    getopt_add_int(gopt, '\0', "KI", "0", "Set the servo's KI (MX/XL)");
    getopt_add_int(gopt, 'm', "compliance-margin", "1", "Set the servo's compliance margin (AX12)");
    getopt_add_int(gopt, 's', "compliance-slope", "32", "Set the servo's compliance slope (AX12)");
    ...
    if (getopt_get_bool(gopt, "home-all")) {
        ...
        if (getopt_was_specified(gopt, "KP")){
            if(servo->protocol == 1){
                ...
                msg->buf[0] = 0x1C;
                msg->buf[1] = getopt_get_int(gopt, "KP") & 0xFF;
                servo->write_to_RAM(servo,msg,0);
                dynamixel_msg_destroy(msg);
            }
            else if(servo->protocol == 2){
        ...
        if (getopt_was_specified(gopt, "KD")){
            if(servo->protocol == 1){
                dynamixel_msg_t *msg = dynamixel_msg_create(2);
                msg->buf[0] = 0x1A;
                msg->buf[1] = getopt_get_int(gopt, "KD") & 0xFF;
                servo->write_to_RAM(servo,msg,0);
                dynamixel_msg_destroy(msg);
            }
            else if(servo->protocol == 2){
        ...
        if (getopt_was_specified(gopt, "KI")){
            ...
            if(servo->protocol == 1){
                dynamixel_msg_t *msg = dynamixel_msg_create(2);
                msg->buf[0] = 0x1B;
                msg->buf[1] = getopt_get_int(gopt, "KI") & 0xFF;
                servo->write_to_RAM(servo,msg,0);
                dynamixel_msg_destroy(msg);
            }
            else if(servo->protocol == 2){
        ...
        if (getopt_was_specified(gopt, "compliance-margin")){
            if(servo->protocol == 1){
                dynamixel_msg_t *msg = dynamixel_msg_create(3);
                msg->buf[0] = 0x1A;
                msg->buf[1] = getopt_get_int(gopt, "compliance-margin") & 0xFF;
                msg->buf[2] = getopt_get_int(gopt, "compliance-margin") & 0xFF;
                servo->write_to_RAM(servo,msg,0);
                dynamixel_msg_destroy(msg);
            }
        }

        if (getopt_was_specified(gopt, "compliance-slope")){
            if(servo->protocol == 1){
                dynamixel_msg_t *msg = dynamixel_msg_create(3);
                msg->buf[0] = 0x1C;
                msg->buf[1] = getopt_get_int(gopt, "compliance-slope") & 0xFF;
                msg->buf[2] = getopt_get_int(gopt, "compliance-slope") & 0xFF;
                servo->write_to_RAM(servo,msg,0);
                dynamixel_msg_destroy(msg);
            }
        }
        ...
        if (getopt_was_specified(gopt, "LED")){
            if(servo->protocol == 1){
                dynamixel_msg_t *msg = dynamixel_msg_create(2);
                msg->buf[0] = 0x19;
                msg->buf[1] = getopt_get_int(gopt, "LED") & 0xFF;
                servo->write_to_RAM(servo,msg,0);
    ...
    if (1) {
        ...
        if (getopt_was_specified(gopt, "id")) {
            ...
            id1 = id0;
            if(getopt_was_specified(gopt, "set-id")){
                id0 = getopt_get_int(gopt, "set-id");
                id1 = id0;
            }
        }
        else {
